...
2026-04-13 の `https://localhost/gitbucket` 実機検証で、GitBucket の label update REST endpoint (`PATCH /api/v3/repos/{owner}/{repo}/labels/{name}`) は 404 を返すことを確認した。次の修正では、`update_label` の REST 404 時に対象 label の存在を `get_label` / `list_labels` で確認したうえで、Web UI fallback (`GET /{owner}/{repo}/issues/labels/{id}/edit`, `POST /{owner}/{repo}/issues/labels/{id}/edit`) を追加する。Web UI が扱えるのは `new_name` と `color` のため、同じリクエストに `description` が含まれる場合は、REST 非対応インスタンスでは structured MCP error で未対応を明示する。

同じ検証で、PR write-path E2E は tool 実行前の git clone 準備が失敗した。GitBucket の `clone_url` は `https://localhost/gitbucket/git/{owner}/{repo}.git` だが、E2E helper が `GITBUCKET_E2E_URL/{owner}/{repo}.git` を組み立てていたためである。PR ツール自体の回帰ではないが、E2E helper は `get_repository.clone_url` を使うように修正し、`create_pull_request` / `add_pull_request_comment` / `merge_pull_request` を再検証する。
