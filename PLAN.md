...
2026-04-13 の `https://localhost/gitbucket` 実機検証で、GitBucket の label update REST endpoint (`PATCH /api/v3/repos/{owner}/{repo}/labels/{name}`) は 404 を返すことを確認した。この修正では、`update_label` の REST 404 時に対象 label の存在を `get_label` / `list_labels` で確認したうえで、Web UI fallback (`GET /{owner}/{repo}/issues/labels`, `POST /{owner}/{repo}/issues/labels/{id}/edit`) を追加する。Web UI が扱えるのは `new_name` と `color` のため、同じリクエストに `description` が含まれる場合は、REST 非対応インスタンスでは structured MCP error で未対応を明示する。
